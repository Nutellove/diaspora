...
describe Webocracy::YesNoMaybePollable do
  before do
  ...
  describe '#is_valid' do
    it 'should accept Decisions with values in {-1,0,1}' do
      ...
      d2 = Webocracy::Decision.new({ :value =>  0 })
      d3 = Webocracy::Decision.new({ :value =>  1 })
      @ynm_pollable << d1
      @ynm_pollable << d2
      @ynm_pollable << d3
    end
    it 'should not accept Decisions with values outside {-1,0,1}' do
      assert_raise Webocracy::InvalidDecision do
        d1 = Webocracy::Decision.new({ :value => 2 })
        @ynm_pollable << d1
      end
      assert_raise Webocracy::InvalidDecision do
        d1 = Webocracy::Decision.new({ :value => -2 })
